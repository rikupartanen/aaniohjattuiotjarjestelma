...
void hunningFunction(int window_size, float (*windows)[window_size]){
  for (int i = 0; i < numberOfWindows; i++) {
...
void FFT(complex float *window, int arraySize){
  if (arraySize <= 1){
      return;
  }
  complex float even[arraySize / 2];
  complex float odd[arraySize / 2];
 ...
 for(int i = 0; i < arraySize; i++){ // split into even and odd positions of the windows
  ...
  for (int i = 0; i < (arraySize / 2); i++){
   ...
   complex float twiddle_factor = cos(2.0 * M_PI * (float)i / (float)arraySize) - I * sin(2.0 * M_PI * (float)i / (float)arraySize); // calculates the twiddle factor
...
void spectrogramFunction(float (*windows)[window_size], float (*spectrogram)[window_size / 2 + 1]){
  complex float window[window_size]; //buffer to store values from windows as complex, needed to perfrom calculations correctly
  complex float fftrest[numberOfWindows][window_size]; // array containing the complex result of FFT

  //saves data from windows float array to window complex float array and get the FFT result
  for (int i = 0; i < numberOfWindows; i++){
    for(int j = 0; j < window_size; j++){
      window[j] =(complex float)windows[i][j]; //saves windows to complex array
    }
...
float windows[numberOfWindows][window_size]; //should be 124 windows, and 256 values in each window array
