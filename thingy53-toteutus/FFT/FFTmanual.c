#include <math.h>     //provides cos(), sin() and cabs() functions
...
numberOfWindows = (sizeof(signal) / sizeof(signal[0])-window_size) / window_step + 1; //should be 124
...
double windows[numberOfWindows][window_size]; //should be about 124 windows(123 from 0), and 256 values(255 from 0)
...
double spectrogram[numberOfWindows][window_size]; // contains the final result after the spectorgramFunction call 
...
void spectrogramFunction(double (*windows)[window_size], double (*spectrogram)[window_size]){
  complex double window[window_size];
  complex double fftrest[numberOfWindows][window_size];

  //caclulates the FFT and saves it to the fftrest array
  for (int i = 0; i < numberOfWindows; i++){
    for(int j = 0; j < window_size; j++){
      window[j] =(complex double)windows[i][j]; //saves it to complex array
    }
  ...
  //calculates the absolute value to get the spectrogram 
  
  for(int i = 0; i < numberOfWindows; i++){
    for(int j = 0; j < window_size; j++){
     spectrogram[i][j] = cabs(fftrest[i][j]); 
     printf("The value is: %.15f, i is: %d, j is: %d\n", spectrogram[i][j], i, j);
    }
  }
}
